...
from optimize import random_search_YZI
...
bounds = [Integer(low=0, high=39, prior='uniform', transform='normalize', name = "Y-grid"),  # Y次元目: 0以上40未満の整数 (0～39)
          ...
          Real(min_input_size , max_input_size , name=f'Input')] 
...
base_dir = f"result/BORS/{input_var}=[{min_input_size}~{max_input_size}]_{trial_base}-{trial_base+trial_num -1}_{current_time}/"
        ...
        for name, variable in src.variables.items():
            ...
            if name == input_var:
                ...
                if pe == pe_this_y:  # y=Grid_yのときに変更処理
                    var[Grid_y, 0, Grid_z] += input_values[2] # (y,x,z)
                dst[name][:] = var
            else:
                dst[name][:] = src[name][:]
...
# # bounds に整数の範囲を指定する
bounds_input = [(min_input_size, max_input_size)]
with open(BO_file, 'w') as f_BO, open(RS_file, 'w') as f_RS:
    for trial_i in range(trial_num):
        ...
        for exp_i in range(exp_size):
            ...
            start = time.time()  # 現在時刻（処理開始前）を取得
            if exp_i == 0:
                best_params, best_score = random_search_YZI(black_box_function, bounds, bounds_input, random_iter_vec[exp_i], f_RS, num_input_grid)
            else:
                np.random.rand(int(cnt_base*num_input_grid)) #同じ乱数列の続きを利用したい
                best_params, best_score = random_search_YZI(black_box_function, bounds, bounds_input, random_iter_vec[exp_i], f_RS, num_input_grid, previous_best=(best_params, best_score))
            end = time.time()  # 現在時刻（処理完了後）を取得
            time_diff = end - start
